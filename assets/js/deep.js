class Deep {
  ...
  getShowList(list) {
    ...
    for (;index < len; index++) {
      ...
      if (this.unit_filter && info && Number(item[1]) * this.unit_filter - Number(info[1]) < 0) {
        break
      }
      info = item
      newlist.push([item[1], item[2]]) // item[1] price ,  item[2] vol
    }
    return newlist
  }
  // 设置画布尺寸
...
Deep.deepStyle = {
  color1: '#4EA800',  // 左侧数组颜色
  color2: '#D8003E',  // 右侧数组颜色
  axisColor: '#45515b', // 坐标轴颜色
  axisFontColor: '#94969C', // 坐标字体颜色
  dotColor1: '#F8B500', // 小圆点颜色
  dotColor2: 'rgba(248, 181,0 , 0.3)',  // 大圆颜色
  textBgColor: '#94969C', // 文本容器颜色
