  ...
  resolveSymbol(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    if (this.productList) {
      let info = this.productList.filter(item => {
        return (item.instrument_id ? item.instrument_id : item.index_id) === Number(num)
      })
      // let px_unit = info.index_id ? (info[0] ? info[0].px_unit : info.px_unit) : info.px_unit
      let px_unit = info[0] ? info[0].px_unit : info.px_unit

      bits = px_unit ? (px_unit.length - px_unit.lastIndexOf('.') - 1) : 8
    }
