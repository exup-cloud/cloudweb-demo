...
let swapsApi = {
  ...
  getContracts(instrumentID) {
    return swapsAxios.get("http://swap.hiup.pro/fe-cov2-api/swap/instruments", {   // 在本地需要用代理请求时候这里需要写全，因为是通过服务端请求
    // return swapsAxios.get(swap + "/instruments", { // 部署上线写成这样
      params: { instrumentID }
    });
  },
  // 获取当前委托和历史委托(已替换新接口)
  ...
  getUserPositionWithCoin(coinCode, status, offset, size) {
    return swapsAxios.get(swap + "/userPositions", {
      params: {
        // contractID,
        coinCode,
  ...
  */
  ...
  // 获取资金流水
  getFlowingWater(data) {
    return swapsAxios.post(swap + "/cashBooks", data);
  },
  /** 领取赠金活动里的奖励
  // name参数解析
  // DepositTradeTask   首次入金+ 成交两笔任务
