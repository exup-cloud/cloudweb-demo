<template>
    <div class='insure-page'>
       ...
       <div class="insure-page-message">
           <p>TigerMEX每个合约都会选取第三方交易所为指数来源，用于标记永续合约，每个指数可能用于合约产品。</p>
          <div class="index-dropdown">
            <drop-down :info="this.list"></drop-down>
          </div>
          ...
          <div class="insure-page-message">
           ...
           <p>
               <span :key="item.instrument_id" v-for="item in info.instruments">{{ item[locale === 'zh-cn' ? 'name_zh' : 'name_en'] }} &nbsp;</span>
           </p>
       </div>
        <div class="insure-page-message">
  ...
  methods: {
    getInfo() {
      this.info = this.list.filter(item => {
        return item.index_id === Number(this.$route.query.id)
      })[0] || {}
      if (this.info.members) {
        let member = this.info.members.filter(item => this.info.base_coin.indexOf(item.coin_code) > -1)[0]
    ...
    init() {
      ...
      let coinPair = this.$route.query.id || this.list[0].index_id
      if (!this.lineType) {
        coinPair = this.productList.filter(item => {
          return Number(item.index_id) === Number(coinPair)
        })
        if (coinPair) {
          coinPair = coinPair[0].instrument_id
        }
      }
