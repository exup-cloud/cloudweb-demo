...
export default {
  ...
  methods: {
    getList() {
      let instrument_id = this.$route.query.id || this.productList[0].instrument_id
      this.deliveryCycle = this.productList.filter(item => { return item.instrument_id === Number(instrument_id) })[0].settlement_interval
      this.swapsApi.fundingrate(instrument_id, (this.getPage - 1) * 10)
      .then(res => {
        ...
        this.name = this.productList.filter(item => {
          return item.instrument_id === Number(instrument_id)
        })[0].symbol
        this.list = res.data
      })
    }
