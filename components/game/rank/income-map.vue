  ...
  methods: {
    selectCurry(time) {
      ...
      if (this.type === 'team') {
        ...
        if (this.tid) {
          this.swapsApi.teamHistoryRanks(this.tid, time).then(res => {
            this.setHistoryList(res.data.ranks)
          })
        } else {
          this.swapsApi.teamHistoryRanks(this.list[0].team_id, time).then(res => {
            this.setHistoryList(res.data.ranks)
          })
        }
        ...
        if (this.uid) {
          this.swapsApi.userHistoryRanks(this.uid, time).then(res => {
            this.setHistoryList(res.data)
          })
        } else {
          this.swapsApi.userHistoryRanks(this.list[0].user_id, time).then(res => {
            this.setHistoryList(res.data)
          })
        }
      }
