            ...
            <div class="info" :key="index" v-for="(item, index) in reduceList" :class="curry(item) ? 'active' : '' " >
              ...
              <div class="detail" v-if="curry(item)">
                <st-row justify="between">
                  <st-row align="center">
                    <div :key="j" v-for="(info, j) in item.product" class="detail-info" :class="productInfo.instrument_id === info.contract_id && 'curry'">
                      ...
                      <p>
                        ...
                        <span :class="item.product[0].ticker.rise_fall_rate < 0 ? 'red' : 'green'">{{ info.ticker.last_price | splitFormat(com.pxUnit) }}</span>
                      </p>
                    </div>
     ...
     .swap-title-top-contract {
       ...
       .list {
         ...
         .detail {
           ...
           height: 32px;
           width: 100%;
          //  background-color: #2a3243;38455D
           background-color: #38455D;
           color: @L9;
            font-size: 12px;
            line-height: 32px;
