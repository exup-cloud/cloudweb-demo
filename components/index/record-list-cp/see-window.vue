     ...
     <table>
         <thead>
             <tr>
                 ...
                 <th>{{ $t('record.cp.clinchPrice') + `(${productInfo.quote_coin})` }}</th>
         ...
         <tbody>
             <tr :key="item.tid" v-for="item in list">
                 <td>
                     <p>
                 ...
                 <td>{{item.px}}</td>
                 <td>{{item.qty}}</td>
                 <td>{{getPositionsValue(item.qty, item.px) |retainDecimals({decimal: com.valueUnit})}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.swapsApi.orderTrades(this.productInfo.instrument_id, this.orderId).then(res => {
