...
<template>
  <div class="confirm-entrust-window">
      <div class="confirm-entrust-window-message">
          ...
          <p><span>{{ $t(`common.${info.way === 1 ? 'buy' : 'sell'}`) }}</span>{{ $t('record.cp.tradeMessage') }}<span>{{ info.category === 1 ? info.price : $t('common.marketPrice') }}</span>{{ info.category === 1 ? productInfo.quote_coin : '' }} <span>{{ info.vol }}</span>{{ $t('common.pieces') + ' ' + productInfo.symbol }} </p>
      </div>
