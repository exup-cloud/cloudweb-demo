          ...
          <thead>
            ...
            <tbody class="current-entrust">
                  <tr :key="index" v-for="(item, index) in list">
                    <td class="btn" style="cursor: pointer;" @click="IdCopy(item.tid)"  :title="'ID:' + item.tid + $t('common.table.clickCopy')"><st-row align="center"><i :class="~[1, 2].indexOf(item.side) ? 'green' : 'red'"></i> <span>{{ productInfo.symbol }}</span></st-row></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.swapsApi.userTrades(this.productInfo.instrument_id, this.offset, 10)
        .then(res => {
