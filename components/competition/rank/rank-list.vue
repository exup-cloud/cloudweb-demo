<template>
  <section class="rank-list">
    <st-row class="rank-list-top">
       <nuxt-link :class="type === 'team' && 'active'" to="/competition/rank/?type=team">战队</nuxt-link>
       <nuxt-link :class="type === 'personal' && 'active'" to="/competition/rank/?type=personal">个人</nuxt-link>
    </st-row>
    <div class="rank-list-bottom">
      <st-row class="rank-time" v-if="type === 'personal'">
        <nuxt-link :class="time === 'day' && 'active'" :to="`/competition/rank/?type=personal&time=day`">日榜</nuxt-link>
        <nuxt-link :class="time === 'week' && 'active'" :to="`/competition/rank/?type=personal&time=week`">周榜</nuxt-link>
        <nuxt-link :class="time === 'total' && 'active'" :to="`/competition/rank/?type=personal&time=total`">总榜</nuxt-link>
      </st-row>
      <div class="rank-list-data">
        ...
        <nuxt-link class="my-team" :to="`/competition/rank/?type=${type}&time=${time}&id=${type==='team'?myTeam.id:myInfo.id}&user_id=${type==='team'?'':myInfo.user_id}&team_id=${type==='team'?myTeam.team_id:myInfo.team_id}`">
        ...
        <div class="list" :class="type === 'team' && 'more'">
          <div class="over">
            <nuxt-link class="item" v-if="index < 50" :class="id === item.id && 'active'" :to="`/competition/rank/?type=${type}&time=${time}&id=${item.id || item.user_id}&user_id=${item.user_id}&team_id=${item.team_id}`" :key="index" v-for="(item, index) in list">
  ...
  methods: {
    getList(time) {
      if (this.$store.state.active.teamList && this.$store.state.active.userList && !time) {
        ...
        let arr = [this.swapsApi.teamsRank('USDT'), this.swapsApi.usersRank('USDT', this.time)]
        if (this.token) {
    ...
    getHistoryInfo() {
      ...
      if (this.type === 'team') {
        ...
        if (this.tid) {
          ...
          this.swapsApi.teamHistoryRanks(this.tid, this.time, 'USDT').then(res => {
          ...
          if (this.list[0] && this.list[0].team_id) {
            this.swapsApi.teamHistoryRanks(this.list[0].team_id, this.time, 'USDT').then(res => {
              //  console.log('teamHistoryRanks####', res)
              this.info = res.data.team_info
        ...
        if (this.uid) {
          ...
          this.swapsApi.userHistoryRanks(this.uid, this.time, 'USDT').then(res => {
          ...
          this.setHistoryInfo(info)
          if (this.list[0] && this.list[0].user_id) {
            this.swapsApi.userHistoryRanks(this.list[0].user_id, this.time, 'USDT').then(res => {
              this.setHistoryList(res.data)
            })
          }
        }
      ...
      .my-team {
        &>div {
          ...
          div {
            ...
            &.img {
              height: 100%;
              background-image: url(../../../assets/img/competition/rank/icon-top.png);
        ...
        .item {
          ...
          .index {
            .img_0,.img_1,.img_2 {
              ...
              // height: 42px;
              height: 100%;
              background-image: url(../../../assets/img/competition/rank/icon-one.png);
              background-position: center center;
              background-repeat: no-repeat;
            }
            .img_1 {
              background-image: url(../../../assets/img/competition/rank/icon-tow.png);
            }
            .img_2 {
              background-image: url(../../../assets/img/competition/rank/icon-three.png);
            }
          }
        }
        }
