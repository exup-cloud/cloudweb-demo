<template>
   <div class="index">
             <main-nav :simulation="true" @submitEntrustInit="submitEntrustInit"></main-nav>
            ...
            <popup :title="$t(`index.open.openTrade`)"  width="520" :callback="closeOpenAccount" v-if="showOpenAccount">
                <open-account-window  :close="closeOpenAccount" :btnText="'开通合约账户，立即领取模拟本金'"></open-account-window>
            </popup>
    </div>
</template>
<script>
...
import MainNav from '../../../components/index/main-nav-one'
...
export default {
  ...
  head() {
    return {
      title: (Utis.addZero(this.ticker.last_price, this.com.pxUnit) || '----') + '  ' + this.productInfo.name_zh + '|TigerMEX-Swap'
    }
  },
  components: {
    MainNav,
  ...
  methods: {
    ...
    // 获取仓位和委托记录
    // getCabinOrHistory() {
    //   this.swapsApi.getUserPositions(this.productInfo.instrument_id, 3).then(res => {
    //     this.$store.commit('market/SET_CABINLIST', {
    //       data: res.data.positions
    //     })
    //   })
    //   this.swapsApi.getUserOrders(this.productInfo.instrument_id, 3).then(res => {
